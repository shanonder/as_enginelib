...
package com.icday.utils
{
	...
	public class DateTimer extends EventDispatcher
	{
		...
		public function DateTimer( delay:Number, repeatCount:int = 0 )
		{
			_baseDelay = delay;
		...
		override public function addEventListener( type:String, listener:Function, useCapture:Boolean = false, priority:int = 0, useWeakReference:Boolean = false ):void
		{
			if( type == TimerEvent.TIMER )
			{
				...
				if( !_baseTimer.hasEventListener( TimerEvent.TIMER ))
					_baseTimer.addEventListener( type, timerHandler ,useCapture , priority , useWeakReference);
			}
			else if( type == TimerEvent.TIMER_COMPLETE )
			{
				...
				if( !_baseTimer.hasEventListener( TimerEvent.TIMER_COMPLETE ))
					_baseTimer.addEventListener( type, timerCompleteHandler ,useCapture , priority , useWeakReference);
			}
		}
