package morn.core.components
{
	...
	import morn.core.plugins.IRTFPlugin;
	import morn.core.plugins.IRichTextFieldCell;
	 ...
	 * <listing>
		...
		//追加文本和显示元素到rtf中
		rtf.append("Hello, World!", [ { index:5, url:SpriteClassA }, { index:13, url:SpriteClassB } ]);
		//替换指定位置的内容为新的文本和显示元素
		rtf.replace(8, 13, "世界", [ { url:SpriteClassC } ]);</listing>
	 * 
	 * 
	 ...
	 * <listing>
		&lt;rtf&gt;
		  ...
		  &lt;sprites&gt;
				&lt;sprite url="SpriteClassA" index="5"/&gt;
				&lt;sprite url="SpriteClassB" index="13"/&gt;
		  &lt;/sprites&gt;
		&lt;/rtf&gt;</listing>
	 */
	public class RichTextField extends TextArea
	{
...
//		public var isHtml:Boolean;
		...
		override protected function createChildren():void {
			...
			_textField = _textRenderer = new TextRenderer();
		 ...
		 * @param	newSprites 要追加的显示元素数组，每个元素包含src和index两个属性，如：{url:sprite, index:1}。
		...
		{			
			...
			if (newText || autoWordWrap)
			{
				...
				if (autoWordWrap && !isHtml) newText += "\n";
				...
				if (isHtml || (autoWordWrap && !isHtml)) textLength = _textRenderer.length - oldLength - 1;
		 ...
		 * @param	newSprites 要替换的显示元素数组，每个元素包含src和index两个属性，如：{url:sprite, index:1}。
		 */
		 ...
		 * @param	newSprite 要插入的显示元素。其格式包含src和index两个属性，如：{url:sprite, index:1}。
		...
		{
			if(obj is IRichTextFieldCell){
				return (obj as IRichTextFieldCell).getDisplay();
			}
...
//				return new clazz() as DisplayObject;				
				var clip:Clip = new Clip(obj as String);
				clip.play();
				return clip;
			...
			}else 
			{
				...
				if(type == "Sprite"){
					var clazz:Class = getDefinitionByName(obj.url) as Class;
					return new clazz() as DisplayObject;
				}else{
					clip = new Clip(obj.url);
					clip.play();
					return clip;
				}
			}
...
//		
		 ...
		 */
		public function get textOnly():String
		{
...
//		}
		...
		public function importXML(data:XML):void
		{
			...
			var sprites:Array = [];
			for (var i:int = 0; i < data.images.image.length(); i++)
			{
				var node:XML = data.images.image[i];
				var type:String = String(node.@type);
				var clazz:Class = RichTextFieldBuilder.getClass(type);
				var sprite:Object;
				if(clazz){
					var cell:IRichTextFieldCell  = new clazz() ;
					sprite = cell;
					cell.importXml(node,isHtml);
				}else{
					sprite = { };
					sprite.url = String(node.@url);
					sprite.type = type;
					if (isHtml) {
