package morn.core.components
{
	...
	internal class SpriteRenderer
	{
		...
		internal function exportXML():XML
		{
			...
			for (var s:* in _spriteIndices)
			{
				if(s is IRichTextFieldCell){
					var info:Object = s;
				}
				if(s is Clip){
					info = { url:(s as Clip).url, index:s.name ,type: "Clip"};
				}else if(s is Sprite){
					info = { url:getQualifiedClassName(s), index:s.name ,type: "Sprite"};
				}
				arr.push(info);
			}
			...
			{
				if(arr[i] is IRichTextFieldCell){
					xml.appendChild((arr[i] as IRichTextFieldCell).exportXml(-i));
				}else{
					var node:XML = <image url={arr[i].url} index={arr[i].index - i} type={arr[i].type} />;
